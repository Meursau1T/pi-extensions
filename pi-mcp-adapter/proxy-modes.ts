...
import { getServerPrefix, isServerEnabled, parseUiPromptHandoff } from "./types.ts";
...
function disabledServerResult(mode: string, serverName: string): ProxyToolResult {
  return {
    content: [{ type: "text" as const, text: `MCP server "${serverName}" is disabled. Use /caps to enable it.` }],
    details: { mode, error: "server_disabled", server: serverName },
  };
}
...
export function executeStatus(state: McpExtensionState): ProxyToolResult {
  ...
  for (const name of Object.keys(state.config.mcpServers)) {
    const enabled = isServerEnabled(state.config, name);
    const connection = state.manager.getConnection(name);
    const metadata = state.toolMetadata.get(name);
    const toolCount = enabled ? metadata?.length ?? 0 : 0;
    const failedAgo = getFailureAgeSeconds(state, name);
    let status = enabled ? "not connected" : "disabled";
    if (!enabled) {
      status = "disabled";
...
export async function executeAuthStart(state: McpExtensionState, serverName: string): Promise<ProxyToolResult> {
  ...
  if (!isServerEnabled(state.config, serverName)) {
    return disabledServerResult("auth-start", serverName);
  }
...
export async function executeAuthComplete(state: McpExtensionState, serverName: string, input: string): Promise<ProxyToolResult> {
  ...
  if (!isServerEnabled(state.config, serverName)) {
    return disabledServerResult("auth-complete", serverName);
  }
...
export function executeSearch(
  ...
  if (server && state.config.mcpServers[server] && !isServerEnabled(state.config, server)) {
    return disabledServerResult("search", server);
  }
...
export function executeList(state: McpExtensionState, server: string): ProxyToolResult {
  ...
  if (!isServerEnabled(state.config, server)) {
    return disabledServerResult("list", server);
  }
...
export async function executeConnect(state: McpExtensionState, serverName: string): Promise<ProxyToolResult> {
  ...
  if (!isServerEnabled(state.config, serverName)) {
    return disabledServerResult("connect", serverName);
  }
...
export async function executeCall(
  ...
  if (serverName && !isServerEnabled(state.config, serverName)) {
    return disabledServerResult("call", serverName);
  }
  ...
  if (!serverName && !toolMeta && prefixMode !== "none") {
    const candidates = Object.keys(state.config.mcpServers)
      .filter(name => isServerEnabled(state.config, name))
