...
import { isServerEnabled, type ToolMetadata } from "./types.ts";
...
export async function initializeMcp(
  ...
  const serverEntries = Object.entries(config.mcpServers).filter(([name]) => isServerEnabled(config, name));
...
export function updateServerMetadata(state: McpExtensionState, serverName: string): void {
  if (!isServerEnabled(state.config, serverName)) return;
...
export function updateStatusBar(state: McpExtensionState): void {
  const ui = state.ui;
  if (!ui) return;
  const enabledNames = Object.keys(state.config.mcpServers).filter((name) => isServerEnabled(state.config, name));
  const total = enabledNames.length;
  if (total === 0) {
    ui.setStatus("mcp", undefined);
    return;
  }
  const connectedCount = enabledNames.filter(
    (name) => state.manager.getConnection(name)?.status === "connected",
  ).length;
...
export async function lazyConnect(state: McpExtensionState, serverName: string): Promise<boolean> {
  if (!isServerEnabled(state.config, serverName)) return false;
  const connection = state.manager.getConnection(serverName);
  if (connection?.status === "needs-auth") {
    return false;
  }
