...
import { isServerEnabled, type McpAuthResult, type McpConfig, type ServerEntry, type McpPanelCallbacks, type McpPanelResult, type ImportKind } from "./types.ts";
import {
...
export async function reconnectServers(
  ...
  if (targetServer && !isServerEnabled(state.config, targetServer)) {
    if (ctx.hasUI) ctx.ui.notify(`Server "${targetServer}" is disabled. Use /caps to enable it.`, "warning");
    return;
  }

  const entries = targetServer
    ? [[targetServer, state.config.mcpServers[targetServer]] as [string, ServerEntry]]
    : Object.entries(state.config.mcpServers).filter(([name]) => isServerEnabled(state.config, name));
...
export async function authenticateServer(
  ...
  if (!isServerEnabled(config, serverName)) {
    const message = `Server "${serverName}" is disabled. Use /caps to enable it.`;
...
function buildMcpPanelCallbacks(
  ...
  return {
    ...
    getConnectionStatus: (serverName: string) => {
      if (!isServerEnabled(config, serverName)) return "disabled";
      const definition = config.mcpServers[serverName];
...
export async function openMcpAuthPanel(
  ...
  const oauthServers = Object.entries(config.mcpServers).filter(
    ([name, definition]) => isServerEnabled(config, name) && supportsOAuth(definition),
  );
