...
function mergeConfigs(base: McpConfig, next: McpConfig): McpConfig {
  ...
  const serverStates = {
    ...(base.settings?.serverStates ?? {}),
    ...(next.settings?.serverStates ?? {}),
  };

  if (settings && Object.keys(serverStates).length > 0) {
    settings.serverStates = serverStates;
  }
...
export type McpServerStateScope = "global" | "project";

export interface McpServerStateOverrides {
  global: Record<string, boolean>;
  project: Record<string, boolean>;
}

function readServerStateMap(filePath: string): Record<string, boolean> {
  const raw = readRawConfigObject(filePath);
  const settings = raw.settings;
  if (!settings || typeof settings !== "object" || Array.isArray(settings)) return {};

  const states = (settings as Record<string, unknown>).serverStates;
  if (!states || typeof states !== "object" || Array.isArray(states)) return {};

  return Object.fromEntries(
    Object.entries(states as Record<string, unknown>).filter(
      (entry): entry is [string, boolean] => typeof entry[1] === "boolean",
    ),
  );
}

export function getServerStateOverrides(
  overridePath?: string,
  cwd = process.cwd(),
): McpServerStateOverrides {
  return {
    global: readServerStateMap(getPiGlobalConfigPath(overridePath)),
    project: readServerStateMap(getProjectPiConfigPath(cwd)),
  };
}

export function updateServerStateOverrides(
  changes: Record<string, boolean | null>,
  scope: McpServerStateScope,
  overridePath?: string,
  cwd = process.cwd(),
): string {
  ...
  const settings = raw.settings && typeof raw.settings === "object" && !Array.isArray(raw.settings)
    ...
    : {};
  const serverStates = readServerStateMap(targetPath);

  for (const [serverName, enabled] of Object.entries(changes)) {
    if (enabled === null) {
      delete serverStates[serverName];
    } else {
      serverStates[serverName] = enabled;
    }
  }

  if (Object.keys(serverStates).length > 0) {
    settings.serverStates = serverStates;
  } else {
    delete settings.serverStates;
  }
