...
import { formatToolName, isServerEnabled, isToolExcluded } from "./types.ts";
...
export function resolveDirectTools(
  ...
  for (const [serverName, definition] of Object.entries(config.mcpServers)) {
    if (!isServerEnabled(config, serverName)) continue;
...
export function createDirectToolExecutor(
  ...
  return async function execute(_toolCallId, params) {
    ...
    if (!isServerEnabled(state.config, spec.serverName)) {
      return {
        content: [{ type: "text" as const, text: `MCP server "${spec.serverName}" is disabled.` }],
        details: { error: "server_disabled", server: spec.serverName },
      };
    }
