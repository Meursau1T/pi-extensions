...
| `serverStates` | Pi-only per-server state map. `false` prevents startup, discovery, proxy calls, and direct-tool registration. Managed by `/caps`. |
...
Per-server `idleTimeout` overrides the global setting. `/caps` writes `serverStates` only to Pi-owned global or project config, so imported server definitions and OAuth data remain untouched.
